...
| **Features** | SP SIERA, SP xFIP, SP K-BB% over the last 30 days, park-adjusted wRC+ against the hand of the opposing starter, bullpen SIERA, home W% vs the opponent's road W%, and the park factor. Every rate is regressed toward league average by sample size and carries a standard error. |
| **Model** | 60/40 blend of a Logistic Regression and a monotonic-constrained `HistGradientBoostingClassifier`. |
...
- **Constants to refresh.** `LeagueConstants` (wOBA weights, FIP constant, league rates) and `PARK_FACTORS` should be refreshed each season.
